...
def run_evaluation():
    ...
    print(f"Initializing {BATCH_SIZE} environments concurrently for Combat Reward Evaluation...")
    env = LuxSapTorchRLEnv(
    ...
    for step in range(MAX_STEPS):
        ...
        for b in range(BATCH_SIZE):
            ...
            # INJECT COMBAT DYNAMICS: 1/6 probability of firing SAP
            act_0[np.random.rand(16) < (1.0/6.0)] = 5
            act_1[np.random.rand(16) < (1.0/6.0)] = 5
        ...
        for b in range(BATCH_SIZE):
            row = {
                ...
                "collision_penalty": float(np.sum(comp["collision_penalty"][b])) if "collision_penalty" in comp else 0.0,
                ...
                "relic_proximity": float(np.sum(comp["relic_proximity"][b])) if "relic_proximity" in comp else 0.0,
                "relic_farming": float(np.sum(comp["relic_farming"][b])) if "relic_farming" in comp else 0.0,
                ...
                "energy_gain": float(np.sum(comp["energy_gain"][b])) if "energy_gain" in comp else 0.0,
                "stagnation_penalty": float(np.sum(comp["stagnation_penalty"][b])) if "stagnation_penalty" in comp else 0.0,
                "combat_kill": float(np.sum(comp["combat_kill"][b])) if "combat_kill" in comp else 0.0,
                "combat_death": float(np.sum(comp["combat_death"][b])) if "combat_death" in comp else 0.0,
...
def generate_report(df):
    ...
    folder_name = f"luxsap_combat_{timestamp}"
    
    # Base directory dynamically resolved to experiments root
    experiments_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../"))
    local_data_dir = os.path.join(experiments_dir, "local-data", folder_name)
    reports_dir = os.path.join(experiments_dir, "reports", folder_name)
    ...
    <html>
    <head>
        <title>Combat Reward Shape [{version}] Analysis</title>
        <style>
    ...
    <body>
        <h1>A/B Combat Reward Shaping Validation ({version_upper})</h1>
    ...
    html_path = os.path.join(reports_dir, "combat_reward_shaping_report.html")
